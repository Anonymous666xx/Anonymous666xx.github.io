// Discord Club - Cloudflare Worker (ES Module)
// Requires KV namespace: DISCORD_CLUB_KV
...
var JWT_SECRET = 'discord-club-jwt-secret-change-me';

var sessions = {};

export default {
  async fetch(request, env) {
    var DISCORD_CLUB_KV = env.DISCORD_CLUB_KV;
    ...
    try {
      if (path === '/ws') {
        return handleWebSocket(request, DISCORD_CLUB_KV);
      }

      if (path === '/api/register' && request.method === 'POST') return handleRegister(request, DISCORD_CLUB_KV);
      if (path === '/api/login' && request.method === 'POST') return handleLogin(request, DISCORD_CLUB_KV);
      if (path === '/api/me' && request.method === 'GET') return handleMe(request, DISCORD_CLUB_KV);
      if (path === '/api/profile' && request.method === 'PUT') return handleUpdateProfile(request, DISCORD_CLUB_KV);

      var channelMatch = path.match(/^\/api\/channels\/([^\/]+)\/messages$/);
      if (channelMatch) {
        if (request.method === 'GET') return handleGetMessages(request, DISCORD_CLUB_KV, channelMatch[1]);
        if (request.method === 'POST') return handleSendMessage(request, DISCORD_CLUB_KV, channelMatch[1]);
      }

      if (path === '/api/friends' && request.method === 'GET') return handleGetFriends(request, DISCORD_CLUB_KV);
      if (path === '/api/friends/request' && request.method === 'POST') return handleFriendRequest(request, DISCORD_CLUB_KV);
      if (path === '/api/friends/accept' && request.method === 'POST') return handleAcceptFriend(request, DISCORD_CLUB_KV);
      if (path === '/api/friends/reject' && request.method === 'POST') return handleRejectFriend(request, DISCORD_CLUB_KV);
      if (path === '/api/friends/requests' && request.method === 'GET') return handleGetPendingRequests(request, DISCORD_CLUB_KV);

      var friendMatch = path.match(/^\/api\/friends\/([^\/]+)$/);
      if (friendMatch && request.method === 'DELETE') return handleRemoveFriend(request, DISCORD_CLUB_KV, friendMatch[1]);
...
async function handleRegister(request, KV) {
  ...
  var existing = await KV.get('users:' + username);
  ...
  await KV.put('users:' + username, JSON.stringify(user));
  await KV.put('users:byId:' + userId, username);
...
async function handleLogin(request, KV) {
  ...
  var data = await KV.get('users:' + username);
...
async function handleMe(request, KV) {
  var user = await authenticate(request, KV);
  if (!user) return jsonResponse({ error: 'Unauthorized' }, 401);
  return jsonResponse({ user: user });
}

async function handleUpdateProfile(request, KV) {
  var user = await authenticate(request, KV);
  if (!user) return jsonResponse({ error: 'Unauthorized' }, 401);
  ...
  await KV.put('users:' + user.username, JSON.stringify(user));
  return jsonResponse({ user: stripSensitive(user) });
}

// ==================== MESSAGES ====================
async function handleGetMessages(request, KV, channelId) {
  var user = await authenticate(request, KV);
  if (!user) return jsonResponse({ error: 'Unauthorized' }, 401);

  var data = await KV.get('messages:' + channelId);
  var messages = data ? JSON.parse(data) : [];
  return jsonResponse(messages.slice(-100));
}

async function handleSendMessage(request, KV, channelId) {
  var user = await authenticate(request, KV);
  if (!user) return jsonResponse({ error: 'Unauthorized' }, 401);
  ...
  var msg = { id: crypto.randomUUID(), author: user.displayName || user.username, authorId: user.id, content: content, channel: channelId, timestamp: Date.now() };

  messages.push(msg);
  await KV.put('messages:' + channelId, JSON.stringify(messages));
...
// ==================== FRIENDS ====================
async function handleGetFriends(request, KV) {
  var user = await authenticate(request, KV);
  if (!user) return jsonResponse({ error: 'Unauthorized' }, 401);

  var data = await KV.get('friends:' + user.id);
  var friends = data ? JSON.parse(data) : [];
  var accepted = friends.filter(function(f) { return f.status === 'accepted'; });

  var enriched = [];
  for (var i = 0; i < accepted.length; i++) {
    var friendData = await KV.get('users:byId:' + accepted[i].friendId);
    if (friendData) {
      var fu = await KV.get('users:' + friendData);
      if (fu) {
...
async function handleGetPendingRequests(request, KV) {
  var user = await authenticate(request, KV);
  if (!user) return jsonResponse({ error: 'Unauthorized' }, 401);
  ...
  for (var i = 0; i < pending.length; i++) {
    var friendData = await KV.get('users:byId:' + pending[i].friendId);
    if (friendData) {
      var fu = await KV.get('users:' + friendData);
      if (fu) {
        var fuj = JSON.parse(fu);
...
async function handleFriendRequest(request, KV) {
  var user = await authenticate(request, KV);
  if (!user) return jsonResponse({ error: 'Unauthorized' }, 401);
  ...
  var targetData = await KV.get('users:' + targetUsername);
  if (!targetData) return jsonResponse({ error: 'User not found.' }, 404);

  var target = JSON.parse(targetData);

  var senderData = await KV.get('friends:' + user.id);
  ...
  await KV.put('friends:' + user.id, JSON.stringify(senderFriends));

  var recipientData = await KV.get('friends:' + target.id);
  ...
  await KV.put('friends:' + target.id, JSON.stringify(recipientFriends));
...
async function handleAcceptFriend(request, KV) {
  var user = await authenticate(request, KV);
  if (!user) return jsonResponse({ error: 'Unauthorized' }, 401);
  ...
  var recipientData = await KV.get('friends:' + user.id);
  var recipientFriends = recipientData ? JSON.parse(recipientData) : [];
  ...
  await KV.put('friends:' + user.id, JSON.stringify(recipientFriends));

  var requesterData = await KV.get('friends:' + requesterId);
  ...
  await KV.put('friends:' + requesterId, JSON.stringify(requesterFriends));
...
async function handleRejectFriend(request, KV) {
  var user = await authenticate(request, KV);
  if (!user) return jsonResponse({ error: 'Unauthorized' }, 401);
...
async function handleRemoveFriend(request, KV, friendId) {
  var user = await authenticate(request, KV);
  if (!user) return jsonResponse({ error: 'Unauthorized' }, 401);

  var userData = await KV.get('friends:' + user.id);
  var userFriends = userData ? JSON.parse(userData) : [];
  userFriends = userFriends.filter(function(f) { return f.friendId !== friendId; });
  await KV.put('friends:' + user.id, JSON.stringify(userFriends));

  var friendData = await KV.get('friends:' + friendId);
  ...
  await KV.put('friends:' + friendId, JSON.stringify(friendFriends));
...
// ==================== WEBSOCKET ====================
function handleWebSocket(request, KV) {
  var url = new URL(request.url);
  ...
  (async function() {
    try {
      ...
      var userData = await KV.get('users:byId:' + userId);
      var user = userData ? JSON.parse(await KV.get('users:' + userData)) : null;
      ...
      client.addEventListener('message', function(e) {
        try {
          ...
          if (msg.type === 'call_offer' && msg.to) {
            msg.from = userId; msg.fromName = user.displayName || user.username;
            sendToUser(msg.to, msg);
          } else if (msg.type === 'call_answer' && msg.to) {
            msg.from = userId; sendToUser(msg.to, msg);
...
function broadcast(data) {
  for (var id in sessions) {
    if (sessions[id].ws && sessions[id].ws.readyState === 1) {
...
async function authenticate(request, KV) {
  ...
  var userData = await KV.get('users:' + payload.username);
